...
public class OdometrySubsystem extends SubsystemBase {
    ...
    private SwerveDrivePoseEstimator3d poseEstimator = new SwerveDrivePoseEstimator3d(
            DrivetrainConstants.DRIVETRAIN_KINEMATICS,
            new Rotation3d(),
            new SwerveModulePosition[4],
            new Pose3d(),
            new Matrix<>(Nat.N4(), Nat.N1(), new double[]{ VisionConstants.WHEEL_TRUST, VisionConstants.WHEEL_TRUST, Math.toRadians(5)}),
            new Matrix<>(Nat.N4(), Nat.N1(), new double[]{ VisionConstants.VISION_TRUST, VisionConstants.VISION_TRUST, Math.toRadians(5)})
    );
     ...
     */
    public void addWheelOdometry(Rotation3d gyroRotation, SwerveModulePosition[] swerveModulePositions) {
    ...
    public void periodic() {
        for (Camera c : this.cameras) {
            if (c.getPoseFieldSpace(this.getRobotPose()).isPresent()) {
                ...
                this.poseEstimator.addVisionMeasurement(c.getPoseFieldSpace(this.getRobotPose()).get(), c.getTimestamp());
            }
        }
    }
}
