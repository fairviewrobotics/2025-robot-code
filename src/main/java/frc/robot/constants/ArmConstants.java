/* Black Knights Robotics (C) 2025 */
package frc.robot.constants;

import edu.wpi.first.math.trajectory.TrapezoidProfile;

public class ArmConstants {
    public static final int PIVOT_MOTOR_ID = 69;
    public static final double PIVOT_P = 0.5;
    public static final double PIVOT_I = 0.0;
    public static final double PIVOT_D = 0.0;
    public static final double PIVOT_MAX_VELOCITY = 5.0;
    public static final double PIVOT_MAX_ACCELERATION = 5.0;
    public static final TrapezoidProfile.Constraints PIVOT_CONSTRAINTS =
            new TrapezoidProfile.Constraints(PIVOT_MAX_VELOCITY, PIVOT_MAX_ACCELERATION);

    public static final double PIVOT_KS = 0.1;
    public static final double PIVOT_KG = 0.0;
    public static final double PIVOT_KV = 0.0;
    public static final double PIVOT_KA = 0.0;

    public static final double PIVOT_TOLERANCE = 1.0;

    public static final double PIVOT_FEED_ANGLE = 90.0;
    public static final double PIVOT_PLACE_ANGLE = 45.0;

    public static final double PIVOT_MAX_ANGLE = 120.0;
    public static final double PIVOT_MIN_ANGLE = 0.0;

    public static final double PIVOT_START_ANGLE = 0.0;

    public static final int LEFT_MOTOR_ID = 0;
    public static final int RIGHT_MOTOR_ID = 0;

    public static final double HAND_P = 0.5;
    public static final double HAND_I = 0;
    public static final double HAND_D = 0;
    public static final double HAND_MAX_VELOCITY = 1.0;
    public static final double HAND_MAX_ACCELERATION = 1.0;
    public static final TrapezoidProfile.Constraints HAND_CONSTRAINTS =
            new TrapezoidProfile.Constraints(HAND_MAX_VELOCITY, HAND_MAX_ACCELERATION);

    public static final int HAND_LINEBREAK_ID = 9;

    public static final double INTAKE_ANGLE = 90.0;
    public static final double PLACE_ANGLE = 45.0;
}
